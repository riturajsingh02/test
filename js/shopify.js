...
(function (window) {
          ...
          try {
            ...
            if (data && typeof data === 'object') {
              ...
              if (Array.isArray(data.addresses)) {
                const clean = data.addresses.filter(a => !isSampleAddress(a));
                if (clean.length !== data.addresses.length) {
                  data.addresses = clean;
                  changed = true;
                }
              }
  ...
  const ShopifyService = {
      ...
      if (stored) {
        try {
          ...
          if (Array.isArray(parsed.addresses)) {
            const clean = parsed.addresses.filter(a => !isSampleAddress(a));
            if (clean.length !== parsed.addresses.length) {
              parsed.addresses = clean;
              changed = true;
            }
          }
          if (isSampleAddress(parsed.defaultAddress)) {
          ...
          if (changed) {
            try {
              ...
              sessionStorage.setItem(CUST_CACHE_KEY, JSON.stringify(parsed));
            } catch (e) {}
          }
          return parsed;
        } catch (e) {}
    ...
    async getDefaultAddress() {
      const cust = await this.getCustomer();
      if (isSampleAddress(cust?.defaultAddress)) return null;
      return cust?.defaultAddress || null;
    },
    ...
    async addAddress(addressData) {
      ...
      const shouldBeDefault = Boolean(addressData.isDefault) || cust.addresses.length === 0;
      const newAddr = {
      ...
      localStorage.setItem(CUST_CACHE_KEY, JSON.stringify(cust));
      sessionStorage.setItem(CUST_CACHE_KEY, JSON.stringify(cust));
      window.dispatchEvent(new CustomEvent('candleier:addressChange', { detail: { customer: cust } }));
      return cust;
    },
    ...
    async updateAddress(addressId, addressData) {
      ...
      if (idx !== -1) {
        const wasDefault = Boolean(cust.addresses[idx].isDefault || cust.defaultAddress?.id === addressId);
        Object.assign(cust.addresses[idx], addressData);

        if (addressData.isDefault || (wasDefault && cust.addresses.length === 1)) {
          cust.addresses.forEach((a, i) => { a.isDefault = (i === idx); });
          cust.defaultAddress = cust.addresses[idx];
        } else if (wasDefault && !addressData.isDefault) {
          // Fallback next address as default
          const nextDef = cust.addresses.find((_, i) => i !== idx) || cust.addresses[idx];
          cust.addresses.forEach(a => { a.isDefault = (a.id === nextDef.id); });
          cust.defaultAddress = nextDef;
        } else if (cust.defaultAddress?.id === addressId) {
          cust.defaultAddress = cust.addresses[idx];
        }
      }
    ...
    async deleteAddress(addressId) {
      ...
      if (cust.defaultAddress?.id === addressId) {
        ...
        if (cust.defaultAddress) {
          cust.defaultAddress.isDefault = true;
          cust.addresses.forEach((a, i) => { a.isDefault = (i === 0); });
        }
      }
